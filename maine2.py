...
root.geometry("1200x800")  # Увеличенный размер окна
...
canvas.pack(pady=20)  # Отступ от верхней части окна

draw_color = "black"
eraser_on = False
eraser_size = 10
brush_size = 3  # Добавлена переменная для размера кисти
original_image = None  # Для хранения оригинального изображения
...
def draw(event):
    if eraser_on:
        ...
        canvas.create_line(x1, y1, x2, y2, fill=draw_color, width=brush_size)  # Использование переменной brush_size
